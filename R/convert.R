...
recompute <- function(calist, mat, rm_zeros = TRUE, top = nrow(mat), ...) {
    ...
    if (isTRUE(call_svd)) {
        ...
        if (u) {
            calist$U <- sweep(
                calist$std_coords_rows,
                1,
                sqrt(calist$row_masses),
                "*"
            )
        }
        if (v) {
...
setMethod(
    ...
    function(obj, ..., assay = "RNA", slot = "counts") {
        ...
        stopifnot(
            "obj doesn't contain a DimReduc object named 'CA'. Try running cacomp()." =
        ...
        ca_list <- list(
            "std_coords_cols" = Seurat::Embeddings(obj, reduction = "CA"),
...
setMethod(
    ...
    function(obj, ..., assay = "counts") {
        ...
        stopifnot(
            "Attribute singval of dimension reduction slot CA is empty.\nThis can happen after subsetting the sce obj." =
                !is.null(attr(sce_ca, "singval"))
        )
        stopifnot(
            "Attribute prin_coords_rows of dimension reduction slot CA is empty.\nThis can happen after subsetting the sce obj." =
                !is.null(attr(sce_ca, "prin_coords_rows"))
        )

        ca_list <- list(
            "std_coords_cols" = sce_ca,
