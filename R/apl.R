...
apl_coords <- function(caobj, group, calc_rows = TRUE, calc_cols = TRUE) {
    ...
    if (is(group, "numeric")) {
        ...
        if (anyNA(idx)) {
            warning(
                "Not all names in 'group' are contained in the column names. ",
...
apl_score <- function(caobj,
    ...
    for (k in seq(reps)) {
        ...
        if (isTRUE(store_perm) & identical(reps, attr(caobj@permuted_data, "reps"))) {
            ...
            if (isTRUE(store_perm)) {
                x <- list(
                    "std_coords_cols" = caobjp@std_coords_cols,
    ...
    ranking <- data.frame(
        "Rowname" = rownames(caobj@apl_rows),
        "Score" = score,
...
apl_topGO <- function(caobj,
    ...
    if (isTRUE(return_plot)) {
        if (top_res > length(results_test@score)) {
            warning(
                "More top nodes selected via top_res than available. ",
...
apl_plotly <- function(rows,
    ...
    p <- p %>%
        plotly::layout(
            title = paste("Association Plot"),
            xaxis = list(
                title = "Distance from origin (x)",
                rangemode = "tozero"
            ),
            yaxis = list(
                title = "Distance from gene to sample line (y)",
                rangemode = "tozero"
            ),
            showlegend = TRUE
        )
...
apl <- function(caobj,
    ...
    if (is.numeric(cols_idx)) {
        group_cols <- data.frame(
            rownms = rownames(caobj@apl_cols)[cols_idx],
    ...
    if (is.numeric(rows_idx)) {
        group_rows <- data.frame(
            rownms = rownames(caobj@apl_rows)[rows_idx],
    ...
    apl_rows.tmp <- data.frame(
        rownms = rownames(caobj@apl_rows),
        caobj@apl_rows
    )
    apl_cols.tmp <- data.frame(
        rownms = rownames(caobj@apl_cols),
        caobj@apl_cols
    )
    ...
    if (isTRUE(show_score)) {
        ...
        if (is.empty(idx)) {
            show_score <- FALSE
            warning(
                "No rows with a Score >= score_cutoff of ",
                score_cutoff,
                ". Choose lower cutoff."
            )
        } else {
    ...
    if (type == "ggplot") {
        ...
        stop(
            "Please specify plot = \"ggplot\" or \"plotly\".",
