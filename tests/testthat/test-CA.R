...
ca_python <- cacomp(obj = as.matrix(smoke), princ_coords = 3, coords = TRUE, python = TRUE)
ca_svd <- cacomp(obj = as.matrix(smoke), princ_coords = 3, coords = TRUE, python = FALSE)
...
test_that("CA with torch svd results", {
  ...
  expect_equal(ca_python@D, smoke_ca$sv)
...
test_that("CA with R svd results", {
  ...
  expect_equal(ca_svd@D, smoke_ca$sv)
