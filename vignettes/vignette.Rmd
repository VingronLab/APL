...
Instead, users can also opt to use the R native SVD. For this, please use the argument `python = FALSE` wherever applicable in this vignette.
...
runAPL(pbmc,
       ...
       show_cols = FALSE)
```
The generated Association Plot is computed based on the log-normalized count matrix using 200 CA dimensions (parameter `dims`). The cluster-specificity score ( \eqn{S_\alpha}) for each gene is also calculated (`score = TRUE`). More information on the choice of dimension number and the score \eqn{S_\alpha} is present in the next section of the vignette.
...
Now we can conduct GO enrichment analysis as implemented in the package `r Biocpkg("topGO")` using the most cluster-specific genes from the Association Plot. By default we use all genes with the positive \eqn{S_\alpha} score, or 1,000 genes with the highest \eqn{S_\alpha} score if there are more than 1,000 genes with the \eqn{S_\alpha} above 0. 
