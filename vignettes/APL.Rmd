...
Once the coordinates for the Association Plot are calculate, they will also be shown in the output of `cacomp`. Slots are accessed through an accessor function:
```{r std_coords}
...
In the scree plot above we can see that the first dimension explains only ~0.75% of the total inertia and we observe the "jump" in the scree plot at roughly 15 dimensions. The first dimensions however explain only a small part of the total inertia. 
