...
py_install(packages = "pytorch")
```

The above might query you to use a python virtualenv. If you prefer to use conda to manage your python packages, you can use the following code:
```{r reticulate_conda, eval=FALSE}
library(reticulate)
install_miniconda()
...
```
IF you use both the virtualenv and the conda environment, after loading reticulate you might have to activate the condaenv with the above `use_condaenv` call in order to use the correct environment.
