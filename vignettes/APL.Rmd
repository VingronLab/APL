...
Here we compute the number of dimensions using the elbow rule. For demonstration, only 1 data permutation is computed, but we highly recommend to set the number of repetitions >= 3 (e.g. `reps = 5`) :
...
bcells <- which(pbmc$cell_type == "3 - B")

# Calculate Association Plot coordinates for B cells from 3k PBMC data
ca <- apl_coords(ca, group = bcells)
```
...
```{r apl_bcells_plot, fig.wide = TRUE}
...
apl(ca,
    row_labs = TRUE,
    rows_idx = bcell_markers, # bcell marker genes
...
Additionally, it is possible to highlight in the Association Plot any set of genes. In the example above we additionally highlighted genes which are known to be marker genes for B cells. The displayed genes are the B cell "Cell Type Enriched" genes as defined by [The Human Protein Atlas](https://www.proteinatlas.org/) that are expressed in the dataset. We added additionally the surface marker "CD19" to the list. As we can see in the plot, they are located in the lower right part of the plot, which confirms their specificity for B cells. 
...
## Association Plots with the \eqn{S_\alpha} scores {#apl}
...
ca <- apl_score(ca,
                ...
                python = TRUE)
```
```{r apl_plot_bcells, fig.wide = TRUE}
apl(ca,
    show_score = TRUE,
    type = "ggplot")
```
...
As expected, the 3 most highly scored genes are over-expressed in the B cell cluster.
...
Additionally, in the Association Plot we can highlight genes of interest identified for instance using the GO enrichment analysis. In the example below we highlight canonical B cell markers.

```{r bcell_apl_score, fig.wide = TRUE}
